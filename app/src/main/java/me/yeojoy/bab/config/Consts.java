package me.yeojoy.bab.config;


public interface Consts {

    public static final String TODAY_MENU_ACTION  = "me.yeojoy.action.TODAY_MENU_ACTION";
    public static final String TOMORROW_MENU_ACTION = "me.yeojoy.action.TOMORROW_MENU_ACTION";
    public static final String AFTER_WORK_MENU_ACTION = "me.yeojoy.action.AFTER_WORK_MENU_ACTION";

    public static final int TODAY_MENU     = 0x000010;
    public static final int TOMMORROW_MENU    = 0x000001;
    
    public static final String KEY_MAIN_MENU            = "main_menu";
    public static final String KEY_SUB_MENU_FIRST       = "sub_menu_1";
    public static final String KEY_SUB_MENU_SECOND      = "sub_menu_2";
    public static final String KEY_SUB_MENU_THIRD_      = "sub_menu_3";
    public static final String KEY_SUB_MENU_FOURTH_     = "sub_menu_4";
}
